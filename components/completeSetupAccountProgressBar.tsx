...
const CompleteSetupProgressBar = ({ step }: CompleteSetupProgressBarProps) => {
  ...
  return (
    <div className="grid grid-cols-3 text-center relative w-[40rem] mx-auto">
      {steps.map((step, index) => (
        <Step key={index} {...step} />
      ))}

      <div className="absolute top-2 left-1/2 transform translate-x-[-50%] border border-gray-300 z-[-1] w-[calc(100%-14rem)]"></div>
    </div>
  );
};
